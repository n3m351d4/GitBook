...
Просматривая блог «Reverse-Engineering Broadcom Wireless Chipsets» сотрудников Quarkslab.com, я заметил, что они выгружают память с помощью крошечного скрипта PySerial, который реализует команду «md» для запроса дампа памяти. Я много раз безуспешно пытался это осуществить. Во время этих попыток я выяснил, что прежде, чем чип распознает вторжение и перезагрузится, прервав выполнение команды, и отключит мой скрипт до записи файла bin, я могу получить только около 2 КБ данных. Я также обнаружил, что если бы я сократил свой дамп до 1 КБ \(1024\), то я мог бы собирать данные генерируя маленькие файлы bin размером 1 КБ, что само по себе не так полезно.
