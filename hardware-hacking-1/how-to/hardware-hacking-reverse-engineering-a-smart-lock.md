...
ВАЖНО: НЕИСПРАВНОСТЬ NRF SNIFFER! 

Похоже, что если блокировка KeyWe выполняет скачок канала после сопряжения, но до того, как приложение передаст 1-й пакет, анализатор nRF пропустит начальный обмен ключами безопасности CCM AES-128 BLE. Это приведет к получению зашифрованных «бесполезных» пакетов. Это недостаток неспособности nRF Sniffer отслеживать карту каналов. Примечание. Обмен ключами безопасности CCM AES-128 BLE - это протокол безопасности, который используется во всех беспроводных соединениях Bluetooth Low Energy OTA для предотвращения прослушивания MiM. Обмен ключами безопасности CCM AES-128 BLE: \(это общая информация, не имеющая отношения к проекту KeyWe\) Временный ключ используется в процессе сопряжения Bluetooth. Краткосрочный ключ используется в качестве ключа для шифрования соединения при первом сопряжении устройств. Краткосрочный ключ генерируется с использованием трех частей информации: временного ключа и двух случайных чисел, одно генерируется ведомым устройством, а другое - ведущим. После того, как соединение зашифровано краткосрочным ключом, остальные ключи распределяются. Долгосрочный ключ заменяет краткосрочный ключ для шифрования соединения. Ключ разрешения личности используется для конфиденциальности. Ключ подписи подключения используется для аутентификации.
