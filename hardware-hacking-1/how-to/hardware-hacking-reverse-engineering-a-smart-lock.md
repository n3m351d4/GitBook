...
Блокировка KeyWe выполняет скачок канала после сопряжения, и до того, как приложение передаст 1-й пакет, анализатор nRF пропускает начальный обмен ключами безопасности CCM AES-128 BLE. Это приводит к получению зашифрованных «бесполезных» пакетов. Это недостаток nRF Sniffer. 
